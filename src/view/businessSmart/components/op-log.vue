<template>
    <div class="logs">
        <Button class="log-btn" @click="toggle">
            日志区
            <Icon type="ios-arrow-down" :class=" fold ? 'rotate180' : 'rotate0' "></Icon>
        </Button>
        <collapse-transition>
            <div class="log-detail" v-show="fold">
                <ul v-show="taskLogs.length" style="list-style: none;">
                    <li v-for="(log,index) in taskLogs" :key="index" style="margin: 10px 0;">
                        <span>{{log}}</span>
                        <!-- <span>{{log.operateTime}}</span> -->
                        <!-- <span
                            style="display: inline-block; margin: 0 0 0 20px ; min-width: 60px;"
                        >{{log.operateUserName}}</span>
