                    ...
                    <Col span="5">
                        ...
                        >
                            <Select v-model="contract.openTicketInfo" filterable clearable>
                                <Option
                                    v-for="item  in openTicketInfoList"
                                    :value="item.tagName"
                                    :key="item.id"
                                    clearable
                                >{{item.tagName}}</Option>
...
export default {
  ...
  data () {
    return {
      ...
      openTicketTypeListTwo: [], // 开票类型
      openTicketInfoList: [], // 开票内容(收款)
      contract: {
  ...
  watch: {
    contractInfo (newVal) {
      ...
      let companyId = newVal.firstCompanyId
      this.getCompanyCard(companyId)
    },
    errorInfo: {
      handler (newVal) {
  ...
  methods: {
    ...
    getContractTime () {
      this.contractInfo.contractStartTime = moment(
        this.contractInfo.contractTime[0]
      ).format('YYYY-MM-DD')
      this.contractInfo.contractEndTime = moment(
        this.contractInfo.contractTime[1]
      ).format('YYYY-MM-DD')
    },
    // 对方联系人输入添加
