...
export default {
  ...
  data () {
    return {
      ...
      channel: {
        ...
        channelSource: '', // 来源
        website: '', // 网址
        companyType: '个人', // 公司/个人
        companyName: '', // 公司名称
        personName: '', // 姓名
        personCardId: '', // 身份证
        contactAddress: '', // 联系地址
        channelType: '下载器', // 渠道类型
        belongTo: '', // 归属
        numberLevel: '', // 量级
        domainName: '', // 域名
        isCloud: '', // 云控
  ...
  watch: {
    channelInfo (newVal) {
      this.channel = newVal
      this.businessPeople = this.channel.marketResponsibleName ? { label: this.channel.marketResponsibleName, id: this.findIdByName(this.treeoptions, this.channel.marketResponsibleName) } : null
      let marketResponsibleName = this.channel.marketResponsibleName || ''
      if (marketResponsibleName.length > 0) {
  ...
  methods: {
    ...
    check () {
      // same 是false，标红
      let data = {
        channel: ['channelSource', 'website']
      }
      let checkChannel = data.channel
      let message = {
        required: true,
        message: '字段不一致',
        trigger: 'blur'
      ...
      checkChannel.map(e => {
        this.ruleChannel[e] = message
        this.channel.e = ''
      })
    },
    // 不同项目标红
    errors (key) {
      let erro = this.channelErro || []
