  ...
  <div class="detailContent">
    <Row class="rowPadding">
      <Col span="2" offset="2" class="title">
          <Tooltip content="标注标签后表示该渠道只上线包含此标签的产品" placement="top" max-width='200' :transfer='true'>
          <Icon type="md-help-circle"  style="margin-right:5px"/></Tooltip>
          <span>端口类型：</span>
      </Col>
      <Col span="5">
        <treeselect
    ...
    <Row style="margin-top:50px">
      <Col span="2" offset="2" class="title">
        <Tooltip content="选择侧重标签后包含该标签的产品推荐优先级更高。" placement="top" max-width='200' :transfer='true'>
          <Icon type="md-help-circle"  style="margin-right:5px"/></Tooltip>
          <span>侧重标签：</span>
      </Col>
      <Col span="5">
...
export default {
  ...
  watch: {
    labelInfo (newV) {
      if (newV) {
        this.productTags = newV
      }
    },
    errorInfo (newV) {
      if (newV) {
        this.errorPort = newV.includes('portType')
        this.errorSoft = newV.includes('softwareFunction')
        this.errorBusiness = newV.includes('businessType')
        this.errorFocus = newV.includes('focusLabels')
      }
    }
  },
  mounted () { this.getSelectData() },
  methods: {
    getSelectData () {
      // 获取端口类型
      this.$axios({
