...
<div class="detailContent">
    <Row class="rowPadding">
        <Col span="16" offset="4">
            <span class="addContractIcon" > +</span><span class="addContract" @click="openModal()">新增开票信息</span><span class="info">建立联系人必填字段</span>
            <Table id="invoiceTable" border :columns="invoicetHeader" :data="invoiceData" :class="[red ?'red':'']">
    ...
    <Modal :mask-closable="false" id="addContract" :loading="loading" v-model="addInvoiceDataModal" @on-ok="saveToTable('invoiceModal')" :title="modalTitle" width="800px">
        <!--开户名 纳税人识别号-->
        <Form ref="invoiceModal" :model="invoiceModal" :rules="ruleInvoiceModal">
        <Row class="rowPadding" style="margin-top:20px;">
            <Col span="4" class="title">
                <i class="require">*</i>开户名：
            </Col>
            <Col span="6">
                <FormItem prop="accountName">
                    <Input v-model="invoiceModal.accountName" placeholder="请输入开户名" clearable filterable maxlength="50" show-word-limit></Input>
                </FormItem>
            </Col>
            ...
            <Col span="7">
                <FormItem prop="identificationNumber">
                    <Input v-model="invoiceModal.identificationNumber" placeholder="请输入纳税人识别号" clearable filterable maxlength="150" show-word-limit></Input>
                </FormItem>
            </Col>
        </Row>
        <!--开户行账号 开户行-->
        <Row class="rowPadding">
            ...
            <Col span="6">
                <FormItem prop="bankAccount">
                    <Input v-model="invoiceModal.bankAccount" placeholder="请输入开户行账号" clearable filterable maxlength="50" show-word-limit></Input>
                </FormItem>
            </Col>
            ...
            <Col span="7">
                <FormItem prop="openingBank">
                    <Input  style="width:220px" v-model="invoiceModal.openingBank" placeholder="请输入银行名称" clearable filterable maxlength="50" show-word-limit></Input>
                </FormItem>
            </Col>
        </Row>
        <!--开票地址 开票电话-->
    ...
    <Modal :mask-closable="false"  v-model="detailInvoiceDataModal" cancel-text=" "  ok-text="关闭" title="详情" width="800px">
        <!--开户名 纳税人识别号-->
        <Row class="rowPadding" style="margin-bottom: 20px;">
            ...
            <Col span="7" style="margin-top:6px">
                <p>{{invoiceModal.identificationNumber}}</p>
            </Col>
        </Row>
        <!--开户行账号 开户行-->
        <Row class="rowPadding"  style="margin-bottom: 20px;">
...
export default {
  ...
  data () {
    return {
      ...
      ruleInvoiceModal: {
        ...
        identificationNumber: [{ required: true, message: '纳税人识别号是必填项哦' }],
        bankAccount: [{ required: true, message: '开户行账号是必填项哦' }],
        openingBank: [{ required: true, message: '开户行是必填项哦' }],
  ...
  watch: {
    ...
    errorInfo (data) {
      if (data) {
        this.red = data.includes('invoiceInfo')
      }
    }
  },
  mounted () {
  ...
  methods: {
          ...
          if (valid) {
            ...
            if (this.$route.query.productCompanyId === undefined) {
              this.myInvoiceData.push({
                ...
                identificationNumber: _self.identificationNumber,
    ...
    geDetail (row, index) {
      ...
      _self.identificationNumber = row.identificationNumber
