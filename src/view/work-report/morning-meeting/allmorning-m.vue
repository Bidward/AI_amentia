...
<div>
    <div class="level-three">
        <div class="searchbar">
            <p v-if="morningSettingPermiss.setMeeting" class="morningSetting" @click="morningSetting()">晨会配置</p>
            <Input @on-search="getTableData()"  @on-clear="getTableData()" v-model="variableObject.searchContent" placeholder="输入关键字..." search clearable />
            <Button icon="ios-loading" @click="refresh()"></Button>
            <TableHeaderSetting
              :totalTableHeader="allTableHeader"
              :tableHeader.sync="tableHeader"
              key="allmorningTabelHeaders"
              localKey="allmorningTabelHeaders"
...
export default {
  ...
  methods: {
    getTableData (currPage, pageSize) {
      ...
      this.$axios({
        url: baseUrl + '/morning/getAllMeetingList',
        method: 'post',
