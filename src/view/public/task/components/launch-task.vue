            ...
            <Col span="20">
              <FormItem prop="acceptUid" label="接收人">
                ...
                >
                  ...
                  >
                    <h3>{{ item.nickname }}</h3>
                    <h4 class="selectDept">部门：{{item.deptName}}</h4>
                    <h4 class="selectDept">职位：{{item.positionName}}</h4>
                  </Option>
                </Select>
              </FormItem>
            </Col>
        ...
        endTime: [{ required: false,
          ...
          validator: (rule, value, callback) => {
            ...
            if (parseInt(type) === 3 && !temp2) {
              ...
              if (moment(endTime).isBefore(moment(sendTime).format('YYYY-MM-DD'))) {
                callback(new Error(`截止时间不能早于发起时间`))
              } else {
                callback()
              }
            }
