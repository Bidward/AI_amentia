...
const track = new Track('综合模块/任务系统/我接收的任务')
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            let lastDay = params.row.endTime || params.row.tasksEndTime
            let todayTimeString = new Date(lastDay + ' 09:00:00').getTime()
            let tomorrowTimeString = 24 * 3600 * 1000 + todayTimeString
            let formatTime = moment(tomorrowTimeString).format('YYYY-MM-DD hh:mm:ss')

            switch (text) {
              case '待确认':
                this.content = `请在${lastConfirm}前与执行人确认可执行方案，执行人确认后，找发起人点击“任务确认”按钮，任务开始进行`
                return (
                  <div>
              ...
              case '进行中':
                this.content = `请在${formatTime}前让执行人完成本阶段所要完成的任务，并让发起人点击“任务完成”按钮`
                return (
                  <div>
                    <span style="display: inline">{text}</span>
                    <Tooltip
  ...
  methods: {
    ...
    getTableData (data) {
      ...
      this.$axios({
        url: `${baseUrl}/tasks/accept/${this.currentTab}`,
        method: 'post',
        data: $qs.stringify(params)
      }).then(res => {
        if (res.data.code === 10000) {
