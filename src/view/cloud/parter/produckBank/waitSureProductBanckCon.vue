  ...
  <div class="viewWrapper">
      <Tabs name="0" value="4"  @on-click="handleClick" :animated="false" class="level-one">
          ...
          <TabPane tab="0" label="产品库" name="4">
              <Tabs name="1" value="1_2" type="card" @on-click="handleInnerClick" :animated="false" class="level-two">
                  <TabPane  tab="1" label="产品库" name="1_1">
                  </TabPane>
                  <TabPane  tab="1" label="待确认产品" name="1_2">
                      <div class="level-three">
                          <div class="searchbar">
                              <Input v-model="headerSearch.text"  placeholder="搜索产品名" search @on-search="getListData()"></Input>
                      ...
                      <Table :loading="loading" :columns="waitCooperHeader" :data="waitCooperContent" border class="cooperTable">
                          <template slot-scope="{ row, index }" slot="action">
                              ...
                              <Button v-if="row.revoke" class="simple marginRight3px" size="small" @click="revokeProduct(row)">撤销</Button>
                              <Button v-if="row.delete" type="error" @click="deleteProductBank(row)"  size="small">删除</Button>
                          </template>
                      </Table>
...
export default {
  ...
  methods: {
    ...
    getListData () {
      ...
      this.$axios({
        url: baseUrl + '/Commodity/getConfirm',
        method: 'post',
    ...
    revokeProduct (row) {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$axios({
            url: baseUrl + '/Commodity/revokeCommodity',
            method: 'post',
            data: $qs.stringify({ tbcId: row.tbcId })
          }).then(res => {
            if (res.data.code === 10000) {
    ...
    // 删除产品公司
    deleteProductBank (row) {
    //   this.$router.push({
    //     name: 'detailProduct',
    //     query: {
    //       productCompanyId: row.commodityId,
    //       type: 'delProdctBanck',
    //       tbcId: row.tbcId
    //     }
    //   })
        ...
        if (row.statusName === '两次信息不一致') {
          ...
          this.$router.push({
            ...
            query: {
              ...
              type: 'tbcEdit', // 第二人编辑
              person: 'oneEntry', // 是发起人来编辑
              msg: 'dif'// 信息不一致
            }
          })
        } else if (row.statusName === '待二次编辑') {
          this.$router.push({
            name: 'editProduckBank',
            query: {
              commodityId: row.commodityId,
