<template>
    <div class="viewWrapper">
        <Tabs name="0" value="1" @on-click="handleClick" :animated="false" class="level-one">
            <TabPane tab="0" label="产品公司" name="1">
                <Tabs name="1" value="1_2"  type="card" @on-click="handleInnerClick" :animated="false"  class="level-two">
                    <TabPane tab="1" label="产品公司" name="1_1">
                    </TabPane>
                    <TabPane tab="1" label="待确认产品公司" name="1_2">
                      <div class="level-three">
                        ...
                        <Table :loading="loading" :columns="waitCooperHeader" :data="waitCooperContent" border>
                            <template slot-scope="{ row, index }" slot="action">
                                ...
                                <Button v-if="row.delete" type="error" @click="deleteProduct(row)"  size="small">删除</Button>
                            </template>
                        </Table>
  ...
  methods: {
    ...
    revokeProduct (row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$axios({
            url: baseUrl + '/productCompany/revoke',
            method: 'post',
