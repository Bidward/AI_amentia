...
export default {
  ...
  methods: {
    ...
    getEchoData () {
      ...
      this.$axios({
        url: baseUrl + '/productCompany/updateinfo',
        method: 'post',
        data: $qs.stringify(sendData)
      }).then(res => {
        if (res.data.code === 10000) {
    ...
    // 获取新增开票信息组件中新增的数据
    getInvoiceMsg (data) {
      // 外层调用组件方法注册变更方法，将组件内的数据变更，同步到组件外的数据状态中
      this.invoiceData = data
    },
    // 根据抄送人id 获得对应的通知人name
    getNoticeName (ids) {
      ids.map(item => {
    ...
    saveNewPro () {
      ...
      let sendData = {
        ...
        tbcId: this.$route.query.productCompanyId,
        twoEntryId: this.mainBusi.twoEntryId, // 通知人
