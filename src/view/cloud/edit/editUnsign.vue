...
<div class="cloudCooper">
    ...
    <div class="detailContent">
        ...
        <!--公司-->
        <div ><!-- v-if="addIncome.contractPartycompany===1" 去除了这条属性,展现甲方-->
            <!--公司/个人 公司名称-->
            <Row class="rowPadding">
    ...
    <div class="detailContent" v-if="isTwo">
                ...
                <div style="width:360px;display:flex;flex-wrap:wrap;">
                  ...
                  <viewer :images="newenclosurePic">
                    ...
                    <div v-for="(item1,index) in newenclosurePic" :key="index+addIncome.enclosurePic.length">
                      <Icon v-if="newenclosurePic.length>=1" type="ios-close-circle-outline" style="position: relative;left: 86px;top: -80px;color: red;" @click="deleteImg(index)" />
                      <img  v-if="newenclosurePic.length>=1" style="width: 70px;height: 70px;margin: 10px;" :src="item1.enclosureUrl" class="inputbox">
                  </div>
                  </div>
                  </viewer>
                </div>
        ...
        <Row class="rowPadding">
            ...
            <Col span="5">
                ...
                <viewer  :images="addIncome.enclosurePic1">
                  <div style="width:360px;display:flex;flex-wrap:wrap;">
                    <div v-for="(item1,index) in addIncome.enclosurePic1" :key="index">
                        <!-- <Icon v-if="addIncome.enclosurePic.length>=1" type="ios-close-circle-outline" style="position: relative;left: 86px;top: -80px;color: red;" @click="deleteImg(index)" /> -->
                        <img  v-if="addIncome.enclosurePic1.length>=1" style="width: 70px;height: 70px;margin: 10px;" :src="item1.enclosureUrl" class="inputbox">
                    </div>
                    <viewer  :images="newenclosurePic">
                      <div style="width:360px;display:flex;flex-wrap:wrap;">
                        <div v-for="(item1,index) in newenclosurePic" :key="index+addIncome.enclosurePic1.length">
                          <Icon v-if="newenclosurePic.length>=1" type="ios-close-circle-outline" style="position: relative;left: 86px;top: -80px;color: red;" @click="deleteImg(index)" />
                          <img  v-if="newenclosurePic.length>=1" style="width: 70px;height: 70px;margin: 10px;" :src="item1.enclosureUrl" class="inputbox">
                        </div>
                      </div>
  ...
  methods: {
    ...
    getData () {
      ...
      if (this.$route.query.type === 'isTwoEntry') {
        ...
        // 待确定合同编辑进入
        if (this.$route.query.type === 'isOneEntry') {
          this.$axios({
            url: baseUrl + '/contract/updateinfo',
            method: 'post',
            data: $qs.stringify({ id: this.$route.query.id })
    ...
    addCopier () {
      this.addIncome.copieruserName = this.showCopierName
      this.addIncome.copier = this.showCopierId
    },
    // 根据通知人id 获得对应的通知人name
    getEntryName (ids) {
      // if (this.addIncome.username === ids.label) {
      //   this.$Message.error('通知人不能是自己哦')
      //   this.flag = false
      // } else {
      //   this.addIncome.twoEntryName = ids.label
      //   this.flag = true
      // }
    ...
    ok () {
      // if (this.flag !== true) {
      //   this.$Message.error('请选择正确通知人')
      //   this.addIncome.twoEntryName = '请重新选择'
      // }
      ...
      if (this.$route.query.type === 'isTwoEntry') {
        if (this.agree !== '') {
          ...
          let sendData = {
            ...
            enclosure: _c.enclosure, // 附件
            enclosurePic: _c.enclosurePir, // 附件图片
            copier: this.addIncome.copier, // 抄送人
            twoEntryId: this.addIncome.twoEntryId, // 通知人ID
            twoEntryName: this.addIncome.twoEntryName, // 通知人Name
        ...
        if (this.addIncome.twoEntryName !== '') {
          ...
          let sendData = {
            ...
            router: 'contractBank',
            tbcId: -1,
            newenclosure: this.newenclosure,
            newenclosurePic: this.newenclosurePic
          }
          for (let i in this.newenclosure) {
            sendData.enclosure.push(this.newenclosure[i])
          }
