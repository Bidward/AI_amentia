...
<div class="detailContent">
    <Row class="rowPadding">
        <Col span="12" offset="3" class="cardTitle">渠道开票信息</Col>
    </Row>
    <Row class="rowPadding">
        <Col span="16" offset="4">
            ...
            <Table id="invoiceTable" border :columns="invoicetHeader" :data="invoiceData">
                 <template slot-scope="{ row, index }" slot="action">
                    <Button type="primary" v-if="$route.name !== 'addProduct'"  size="small" style="margin-right: 5px" @click="edit(row,index)">编辑</Button>
                    <Button type="error" v-if="invoiceData.length>1" size="small" style="margin-right: 5px" @click="deleteRow(row,index)">删除</Button>
                </template>
            </Table>
        </Col>
    </Row>
        ...
        <Row class="rowPadding">
            ...
            <Col span="6">
                <FormItem prop="openingBankCard">
                    <Input v-model="invoiceModal.openingBankCard" placeholder="请输入开户行账号" clearable filterable></Input>
                </FormItem>
            </Col>
            ...
            <Col span="8">
                <FormItem prop="openingBank">
                    <Input  style="width:200px" v-model="invoiceModal.openingBank" placeholder="请输入开户行" clearable filterable></Input>
                </FormItem>
            </Col>
        </Row>
        <!--开票地址 开票电话-->
...
export default {
  ...
  data () {
    return {
      ...
      ruleInvoiceModal: {
        accountName: [{ required: true, message: '开户行是必填项哦' }],
      ...
      if (this.modalTitle === '新增开票信息') {
        this.$refs.invoiceModal.validate((valid) => {
          if (valid) {
            ...
            let _self = this.invoiceModal
            if (this.$route.query.productChannelId === undefined) {
