...
<div class="cloudCooper">
    <Form ref="addIncome" :model="addIncome" :rules="ruleAddIncome">
        ...
        <div class="detailContent">
            ...
            <!--开票类型 开票内容-->
            <Row class="rowPadding" v-if="addIncome.invoice === 1">
                <Col span="3" offset="3" class="title">
        ...
        <div class="detailContent">
            ...
            <Row class="rowPadding" v-if="addIncome.contractPartycompany ===2">
                ...
                <Col span="5">
                <FormItem prop="contractPartycompany">
                    <Select v-model="addIncome.contractPartycompany"   @on-change="choseCompanyType">
                        <Option v-for="item  in addIncome.contractPartycompanyList" :value="item.id" :key="item.id">{{item.name}}</Option>
                    </Select>
                </FormItem>
                </Col>
        ...
        <div class="detailContent">
            ...
            <Row class="rowPadding">
                ...
                <Col span="5">
                <Upload class="bzUpload" multiple name="fileData" :max-size="20480"  :on-success="(value)=> uploadFileSuccess(value)" :on-exceeded-size="uploadFileError" :action="staticUrl+'/index.php'">
                ...
                <viewer  :images='addIncome.enclosurePic'>
                     <div style="width:360px;display:flex;flex-wrap:wrap;">
                        <div v-for="(item,index) in addIncome.enclosurePic" :key="index">
                          <Icon v-if="addIncome.enclosurePic.length>=1" type="ios-close-circle-outline" style="position: relative;top: -61px;left: 80px;color: red;" @click="deleteImg(index)" />
        ...
        <div class="detailContent">
            ...
            <!--通知人弹窗-->
            <Modal :mask-closable="false" id="addNotice" v-model="showNotice" title="通知人" @on-ok="addCopier()">
  ...
  methods: {
    ...
    getContractNum (ids) {
      ...
      if (this.addIncome.contractPartycompany === 1) {
        ...
        // console.log('公司')
        let sendData = {
          productChannelId: this.addIncome.partyCompanyId
        }
        this.$axios({
