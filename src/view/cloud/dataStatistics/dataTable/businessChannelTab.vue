...
export default {
  ...
  data () {
    return {
      ...
      dataTab: { // 表格数据
        ...
        cooperationProductNum: [], // 合作的产品数量
        productMatchcooperation: [], // 产品匹配合作率
        channelOnlineNum: []// 渠道在线链接数量
      }
    }
  },
  mounted () {
  ...
  methods: {
    getData (sendData) {
      this.$axios({ // 调用后端市场渠道数据接口,获取数据赋值给表格数据
        url: baseUrl + '/bzCloudData/BusinessChannelData',
        method: 'post',
        ...
        if (res.data.code === 10000) {
          ...
          if (_p.length === 0) {
            ...
            _p.map((item) => {
              ...
              _this.dataTab.addChannel.push(item.addNum)
              _this.dataTab.matchProductNum.push(item.matchNum)
              _this.dataTab.cooperationProductNum.push(item.cooperationNum)
              _this.dataTab.productMatchcooperation.push(item.cooperationRate.replace('%', ''))
              _this.dataTab.channelOnlineNum.push(item.linkNum)
            })
    ...
    echartOption () {
      ...
      _this.option = { // echarts功能
        ...
        legend: {// 图标上方的标题展示
          data: ['新增渠道', '合作的产品数量', '匹配的产品数量', '产品匹配合作率', '渠道在线链接数量']
        },
        grid: {
          containLabel: false,
          height: 'auto'
        ...
        series: [ // 每一个柱形
          ...
          },
          {
            ...
            type: 'line',
            yAxisIndex: 0,
            data: _this.dataTab.productMatchcooperation
          },
          {
            name: '渠道在线链接数量',
            type: 'bar',
            yAxisIndex: 0,
            data: _this.dataTab.channelOnlineNum
          }
        ]
      }
    }
