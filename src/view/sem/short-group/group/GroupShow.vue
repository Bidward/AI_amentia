<template>
  <div class="viewWrapper viewWrapperNoTabs" id="editExtension">
    <Form ref="editExtension" :label-width="80" :model="extensionData">
      ...
      <div class="selection" v-if="false">
        <h3>导航创意</h3>
        <Row>
  ...
  methods: {
    getData (id) {
      this.$axios({
        ...
        this.extensionData.phraseKeywords = res.data.content.phraseKeywords.replace(
          /,/g,
          '\n'
        )
        this.extensionData.exactKeywords = res.data.content.exactKeywords.replace(
          /,/g,
          '\n'
        )
      })
