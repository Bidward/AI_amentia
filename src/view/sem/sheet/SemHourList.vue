...
const track = new Track('营销/360成本表/小时')
export default {
  name: 'semHourList',
  components: {
    TableHeaderSetting
    ...
    return {
      ...
      totalTableHeader: [
        ...
        },
        {
          ...
          render: (h, { row, params, index }) => {
            return (
              <span class={this.keyWordIncludeSoftNameColor(row.softName, row.keyword)}>{row.keyword}</span>
            )
          }
        },
        {
            ...
            return (
              <span>
                <Poptip placement="bottom">
                  <div slot="content">
                    <Input
                      v-model={this.detailHeader.xzShowid}
                      search
                      clearable
                      v-on:on-search={value => {
                        this.detailSearch({ xzShowid: value, currPage: 1 })
                      }}
                    />
                  </div>
                  <span class="pointer">
                    ...
                    <Icon
                      type="md-search"
                      size="14"
                      class={this.detailHeader.xzShowid ? 'on' : ''}
                    />
                  </span>
                </Poptip>
              </span>
  ...
  methods: {
    ...
    exportExcel () {
      ...
      window.open(semUrl + '/CostT/hour?' + data)
    },
    // 搜索
