        ...
        <Row>
          <Col span="12">
            <FormItem label="产品名" prop="softName">
              <Input :disabled="isEdit" v-model="extensionData.softName" class="wd80" />
            </FormItem>
          </Col>
        </Row>
        <Row v-if="!isEdit">
...
export default {
  ...
  methods: {
    ...
    uploadSuccess (type, index, res, file, fileList) {
      if (res.rel === true) {
        if (type === 'picUrl') {
          this.extensionData.picUrl.push('http://zone.bz.cn' + staticUrl + res.msg)
        } else {
          this.extensionData.windows[index].imageurl = 'http://zone.bz.cn' + staticUrl + res.msg
        }
      } else {
