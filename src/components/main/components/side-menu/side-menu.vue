      ...
      <template v-for="item in menuList">
        <template v-if="item.children && item.children.length === 1">
          ...
          <menu-item v-else :name="getNameOrHref(item, true)" :key="`menu-${item.children[0].name}`">
            ...
            <!-- <span v-if="item.name==='_home'" class="mainTips">{{waitHandel}}</span> -->
          </menu-item>
        </template>
        <template v-else>
          <side-menu-item v-if="showChildren(item)" :key="`menu-${item.name}`" :parent-item="item"></side-menu-item>
...
export default {
  ...
  data () {
    return {
      openedNames: []
      // waitHandel: sessionStorage.getItem('waitHandel') || 0
    }
  },
  methods: {
    handleSelect (name) {
      this.$emit('on-select', name)
      // this.waitHandel = sessionStorage.getItem('waitHandel')
    },
  ...
  // created () {
  //   bus.$on('waitHandel', res => this.waitHandel = res)
  // },
  mounted () {
